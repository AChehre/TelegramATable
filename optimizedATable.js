...
const pack = (t, q, a, p, s, r = "") =>
  `${CallbackActions.Table};${t};${q};${a};${p};${s};${r}`;
...
function createTable(tableName, queryKey, tableData, page = 1, pageSize = PageSize) {
  ...
  for (const r of tableData.rows) {
    ...
    for (const cell of r.cells) {
      const cd = cell.actionKey
        ? pack(tableName, queryKey, cell.actionKey, page, pageSize, rowKey)
        : ignore(tableName);
  ...
  rows.push([
    prevBtn,
    { text: `صفحه ${page} از ${pageCount}`, callback_data: ignore(tableName) },
    nextBtn,
  ]);
